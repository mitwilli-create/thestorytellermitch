...
4. Read the last lines of `output/run-manifest.json` for the run's estimated spend, itemized per call.
...
- You want a cheap draft first: `node pipeline.mjs --skip-gen` skips the visual-generation spend (mograph renders and color cards stand in for generated shots) while the audio stages run normally, so you can check the cut's rhythm for pennies. For a fully free rehearsal, use `--mock`.
...
The finished mp4, the run manifest, and one sentence on what you would change next run. The manifest is the itemized ledger; the sentence is the direction.
