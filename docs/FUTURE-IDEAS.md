# Future Ideas: Portfolio (storytellermitch.com)

Intake log for storytellermitch.com portfolio-site ideas. One entry per idea, newest appended at the bottom, entries separated by `---`. This header is the intake contract; the scheduled job that writes here follows it exactly.
...
- **Idempotency.** Before appending, the log is checked for an existing entry capturing the same idea; duplicates are cross-referenced, not re-added. The `Portfolio Logged` Gmail label additionally prevents reprocessing.
...
### Summary
2-3 plain sentences, faithful to what the email actually said.

### Why it matters
The strategic reason this is worth doing.

### Rough shape
Bulleted sketch of what it could involve. A sketch, not a spec.

### Open questions for first strategy conversation
The real questions to answer before scoping: sequencing, dependencies, unknowns, tradeoffs, cost, success metric. Concrete and grounded in the idea, not boilerplate.
```
