#!/usr/bin/env node
// 8s steady-state hero loop generator (generalized from the timeline-c
// one-shot): --page <name> --still <path> --motion "<prompt>". Produces
// fill-<page>-hd.{mp4,webm} 1920 plates + fill-<page>-loop.{mp4,webm}
// 960w variants + fill-<page>-hd.jpg poster, through the standing gates:
// seam scdet <2 at the cycle boundary (refuses an unmeasured seam),
// palette vs the source still, luma span <=3.5 across the cycle, and a
// montage written for the mandatory eyes-on review.
const argv = (flag, fallback) => { const i = process.argv.indexOf(flag); return i > -1 ? process.argv[i + 1] : fallback; };
...
console.log('[hero-8s] veo3.1 fast i2v 8s...');
...
console.log(`[hero-8s] req ${vid.requestId} est $${vid.estCostUsd.toFixed(3)}`);
...
console.log('[hero-8s] raw dims:', probe);
...
console.log(`[hero-8s] seam=${seam.toFixed(3)} (gate <2) palette=${JSON.stringify(palette)} luma=[${yMin.toFixed(1)},${yMax.toFixed(1)}] (span gate <=${LUMA_SPAN_MAX})`);
if (seam >= 2 || !palette.pass || (yMax - yMin) > LUMA_SPAN_MAX) { console.error('[hero-8s] FAILED gates; inspect ' + work); process.exit(2); }
...
const enc = (w, crf264, crfVp9, name) => {
  ...
  console.log(`[hero-8s] ${name}: mp4=${Math.round(statSync(mp4).size / 1024)}KB webm=${Math.round(statSync(webm).size / 1024)}KB`);
};
enc(1920, 23, 40, `fill-${PAGE}-hd`);
enc(960, 26, 34, `fill-${PAGE}-loop`);
ff(['-ss', '0', '-i', loop, '-frames:v', '1', '-q:v', '3', resolve(SITE, 'assets/stills', `fill-${PAGE}-hd.jpg`)]);
console.log('[hero-8s] DONE; montage at', montage);
